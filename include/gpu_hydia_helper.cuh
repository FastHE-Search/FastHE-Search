...
{
public:
    GPUHydiaHelper(CryptoContext<DCRTPoly> cc,
                   const KeyPair<DCRTPoly> &keys,
                   const std::vector<int> &gpuDevices = {0},
                   int batchSize = 8,
                   double gpuSafeMemoryLimitGB = -1.0);
     ...
     */
    void CacheDiagonalsWithPreRotationOnGPU(
     ...
     */
    bool StreamDiagonalsToGPU(const std::string &serialDir,
     ...
     */
    void CachePreRotatedDiagonalsOnGPU(
        const std::vector<Ciphertext<DCRTPoly>> &diagonals,
        size_t numMatrices, size_t vectorDim, int n1);
     ...
     */
    bool StreamPreRotatedDiagonalsToGPU(const std::string &serialDir,
                                        size_t numDiagonals, size_t numMatrices, int n1);

    bool areDiagonalsCached() const;
    bool areDiagonalsPreRotated() const { return diagonalsPreRotated_; }
    int getPreRotationN1() const { return preRotationN1_; }
     ...
     */
    void ComputeAndCacheBabyStepsOnGPU(
     ...
     */
    Ciphertext<DCRTPoly> EvalOnDemandBSGSLazyChebyshevAndAggregateOnGPU(
        size_t numMatrices, size_t vectorDim, int n1,
     ...
     */
    std::vector<Ciphertext<DCRTPoly>> EvalOnDemandBSGSLazyChebyshevBatchedOnGPU(
        size_t numMatrices, size_t vectorDim, int n1,
        const Ciphertext<DCRTPoly> &templateCt,
     ...
     */
    Ciphertext<DCRTPoly> EvalSimilarityChebyshevAndAggregateOnGPU(
     ...
     */
    std::vector<Ciphertext<DCRTPoly>> EvalSimilarityAndChebyshevBatchedOnGPU(
        size_t numMatrices, size_t vectorDim,
        const Ciphertext<DCRTPoly> &templateCt,
        double delta, size_t signDepth);
    ...
    bool isReady() const { return gpuContextReady_; }
    bool isGPUMemoryInsufficient() const { return gpuMemoryInsufficient_; }
    double getGPUSafeMemoryLimitGB() const { return gpuSafeMemoryLimitGB_; }
    ...
    void *EvalChebyshevSeriesPSOnGPU(
        void *gpuCt,
        const std::vector<double> &coefficients);

    void *EvalChebyshevCompareOnGPU(
        void *gpuCt,
        double delta,
        size_t signDepth);
...
private:
    ...
    void *ProcessSimilarityMatrixOnStream(
