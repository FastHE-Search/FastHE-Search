...
struct GPUTransferStats {
    ...
    void printSummary() const {
        ...
        double avgCpuToGpu = numCPUtoGPU_transfers > 0 ? 
            (totalCPUtoGPU_bytes.load() / (double)numCPUtoGPU_transfers.load()) / 1024.0 : 0;
        double avgGpuToCpu = numGPUtoCPU_transfers > 0 ? 
            (totalGPUtoCPU_bytes.load() / (double)numGPUtoCPU_transfers.load()) / 1024.0 : 0;
        
        std::cout << std::fixed << std::setprecision(2);
        std::cout << "║ CPU → GPU      │ " << std::setw(9) << numCPUtoGPU_transfers.load() 
                  << " │ " << std::setw(10) << cpuToGpuMB << " MB │ " 
                  << std::setw(10) << avgCpuToGpu << " KB     ║" << std::endl;
        std::cout << "║ GPU → CPU      │ " << std::setw(9) << numGPUtoCPU_transfers.load() 
                  << " │ " << std::setw(10) << gpuToCpuMB << " MB │ " 
                  << std::setw(10) << avgGpuToCpu << " KB     ║" << std::endl;
        std::cout << "╠════════════════╧═══════════╧════════════════╧═══════════════════╣" << std::endl;
...
public:
    ...
    void printSummary() const {
        ...
        for (const auto& entry : entries_) {
            ...
            std::cout << std::fixed << std::setprecision(2);
            std::cout << "║ " << std::left << std::setw(30) << entry.name.substr(0, 30) 
                      << " │ " << std::right << std::setw(11) << entry.duration_ms 
                      << " │ " << std::setw(5) << entry.count 
                      << " │ " << std::setw(8) << avg << " ║" << std::endl;
...
#define GPU_TRACK_CPU_TO_GPU(bytes, type, desc) do { \
    getTransferStats().recordCPUtoGPU(bytes, type); \
    if (GPU_DEBUG_TRANSFER) { \
        std::cout << "[GPU_TRANSFER] CPU→GPU: " << (bytes) << " bytes (" \
                  << ((bytes) / 1024.0) << " KB) [" << (type) << "] " << (desc) << std::endl; \
    } \
} while(0)

#define GPU_TRACK_GPU_TO_CPU(bytes, type, desc) do { \
    getTransferStats().recordGPUtoCPU(bytes, type); \
    if (GPU_DEBUG_TRANSFER) { \
        std::cout << "[GPU_TRANSFER] GPU→CPU: " << (bytes) << " bytes (" \
                  << ((bytes) / 1024.0) << " KB) [" << (type) << "] " << (desc) << std::endl; \
    } \
} while(0)
...
// Debug print macros
#define GPU_DEBUG_PRINT(msg) do { \
    std::cout << "[GPU_DEBUG] " << msg << std::endl; \
} while(0)

#define GPU_DEBUG_PRINT_VERBOSE(msg) do { \
...
#define GPU_DEBUG_CT_INFO(name, N, L, level) do { \
    if (GPU_DEBUG_CIPHERTEXT_INFO) { \
        size_t ctSize = 2 * (L) * (N) * 8; \
        std::cout << "[GPU_CT_INFO] " << (name) << ": N=" << (N) << " L=" << (L) \
                  << " level=" << (level) << " size=" << (ctSize / 1024.0) << " KB" << std::endl; \
    } \
} while(0)

// Memory tracking macro
#define GPU_DEBUG_MEMORY(msg, bytes) do { \
    if (GPU_DEBUG_MEMORY_ALLOC) { \
        std::cout << "[GPU_MEMORY] " << msg << ": " << (bytes) << " bytes (" \
                  << ((bytes) / (1024.0 * 1024.0)) << " MB)" << std::endl; \
    } \
} while(0)

// Reset all stats
#define GPU_PROFILER_RESET() do { \
...
// Print all summaries
#define GPU_PROFILER_PRINT_SUMMARY() do { \
...
public:
    ...
    ~GPUScopedTimer() {
        ...
        std::cout << "[GPU_TIMER] <<< " << name_ << ": " << std::fixed 
