...
size_t computeRequiredDepth(size_t approach);
...
vector<double> decryptVectorToVector(CryptoContext<DCRTPoly> cc, PrivateKey<DCRTPoly> sk, vector<Ciphertext<DCRTPoly>> ctxt);

Ciphertext<DCRTPoly> binaryRotate(CryptoContext<DCRTPoly> cc, Ciphertext<DCRTPoly> ctxt, int factor);

Ciphertext<DCRTPoly> sign(CryptoContext<DCRTPoly> cc, Ciphertext<DCRTPoly> ctxt, size_t maxDepth);

Ciphertext<DCRTPoly> sumAllSlots(CryptoContext<DCRTPoly> cc, Ciphertext<DCRTPoly> ctxt);

Ciphertext<DCRTPoly> chebyshevCompare(CryptoContext<DCRTPoly> cc, Ciphertext<DCRTPoly> ctxt, double delta, size_t signDepth);
...
Ciphertext<DCRTPoly> chebyshevCompare(CryptoContext<DCRTPoly> cc, Ciphertext<DCRTPoly> ctxt, double delta, size_t signDepth, double lowerBound, double upperBound);
...
vector<Ciphertext<DCRTPoly>> compressCiphers(CryptoContext<DCRTPoly> cc, vector<Ciphertext<DCRTPoly>>& ctxts, size_t dimension);
} // namespace OpenFHEWrapper