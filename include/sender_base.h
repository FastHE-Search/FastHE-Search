...
class BaseSender : public HersSender {
  ...
  protected:
	void computeSimilarityThread(Ciphertext<DCRTPoly>& queryCipher, Ciphertext<DCRTPoly>& similarityCipher, size_t databaseIndex);

	void computeSimilarityAndMergeThread(Ciphertext<DCRTPoly>& queryCipher, Ciphertext<DCRTPoly>& mergedCipher, size_t startingIndex);

	vector<Ciphertext<DCRTPoly>> computeSimilarityAndMerge(Ciphertext<DCRTPoly>& queryCipher);
};