...
DiagonalSenderGPU::DiagonalSenderGPU(CryptoContext<DCRTPoly> ccParam, PublicKey<DCRTPoly> pkParam, size_t vectorParam, GPUHydiaHelper* gpuHelper)
...
void DiagonalSenderGPU::cacheDiagonalsOnGPU() {
	...
	cout << "[Approach 51] Cached " << allDiagonals.size() << " diagonals on GPU in " << fixed << setprecision(2) << elapsed << "s" << endl;
}
...
void DiagonalSenderGPU::streamDiagonalsFromDisk(const string& serialDir) {
	...
	if (success) {
		gpuCacheInitialized_ = true;
		cout << "[Approach 51] Streamed " << (numMatrices * VECTOR_DIM) << " diagonals disk→GPU in " << fixed << setprecision(2) << elapsed << "s" << endl;
	} else {
...
Ciphertext<DCRTPoly> DiagonalSenderGPU::membershipScenario(vector<Ciphertext<DCRTPoly>>& queryCipher) {
	...
	Ciphertext<DCRTPoly> membershipCipher =
	  gpuHelper_->EvalSimilarityChebyshevAndAggregateOnGPU(numMatrices, VECTOR_DIM, queryCipherForClone_, MATCH_THRESHOLD, COMP_DEPTH_GPU, batchSize);
...
vector<Ciphertext<DCRTPoly>> DiagonalSenderGPU::indexScenario(vector<Ciphertext<DCRTPoly>>& queryCipher) {
	...
	vector<Ciphertext<DCRTPoly>> scoreCipher =
	  gpuHelper_->EvalSimilarityAndChebyshevBatchedOnGPU(numMatrices, VECTOR_DIM, queryCipherForClone_, MATCH_THRESHOLD, COMP_DEPTH_GPU);
