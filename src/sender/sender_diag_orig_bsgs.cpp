...
DiagonalOrigBSGSSender::DiagonalOrigBSGSSender(CryptoContext<DCRTPoly> ccParam, PublicKey<DCRTPoly> pkParam,
               size_t vectorParam)
...
Ciphertext<DCRTPoly> DiagonalOrigBSGSSender::computeSimilarityMatrixBSGS(
...
Ciphertext<DCRTPoly> DiagonalOrigBSGSSender::computeSimilarityThreadBSGS(
    vector<Ciphertext<DCRTPoly>> &babySteps,
    vector<shared_ptr<vector<DCRTPoly>>> &babyStepsPrecomp,
  ...
  if (giantIdx == 0) {
    ...
    rotatedQuery = cc->EvalFastRotation(babySteps[babyIdx], giantIdx * babyStepSize, 
                                         cyclotomicOrder, babyStepsPrecomp[babyIdx]);
  }
