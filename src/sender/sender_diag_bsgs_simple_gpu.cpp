...
DiagonalBSGSSimpleSenderGPU::DiagonalBSGSSimpleSenderGPU(
    ...
    : DiagonalBSGSSimpleSender(ccParam, pkParam, vectorParam),
      gpuHelper_(gpuHelper),
      gpuCacheInitialized_(false),
      rotationsCached_(false)
{
...
void DiagonalBSGSSimpleSenderGPU::cacheDiagonalsOnGPU() {
    ...
    cout << "[Approach 81] Cached " << allDiagonals.size() 
         << " diagonals with pre-rotation (n1=" << babyStepSize << ") on GPU in " 
...
void DiagonalBSGSSimpleSenderGPU::cacheDiagonalsOnGPU(const vector<Ciphertext<DCRTPoly>>& preloadedDiagonals) {
    ...
    cout << "[Approach 81] Caching " << preloadedDiagonals.size() 
         << " preloaded diagonals with pre-rotation on GPU..." << endl;
    auto startTime = chrono::steady_clock::now();
    ...
    auto endTime = chrono::steady_clock::now();
    double elapsed = chrono::duration<double>(endTime - startTime).count();
    cout << "[Approach 81] Cached diagonals with pre-rotation (n1=" << babyStepSize 
...
void DiagonalBSGSSimpleSenderGPU::streamDiagonalsFromDisk(const string& serialDir) {
    ...
    if (success) {
        gpuCacheInitialized_ = true;
        cout << "[Approach 81] Streamed " << (numMatrices * VECTOR_DIM)
             << " diagonals disk→GPU with pre-rotation (n1=" << babyStepSize 
...
Ciphertext<DCRTPoly> DiagonalBSGSSimpleSenderGPU::membershipScenario(
    vector<Ciphertext<DCRTPoly>> &queryCipher) 
{
    ...
    try {
        membershipCipher = gpuHelper_->EvalOnDemandBSGSLazyChebyshevAndAggregateOnGPU(
            numMatrices, VECTOR_DIM, babyStepSize, queryCipherForClone_, 
            MATCH_THRESHOLD, COMP_DEPTH_GPU, batchSize);
...
vector<Ciphertext<DCRTPoly>> DiagonalBSGSSimpleSenderGPU::indexScenario(
    vector<Ciphertext<DCRTPoly>> &queryCipher) 
{
    ...
    vector<Ciphertext<DCRTPoly>> scoreCipher = gpuHelper_->EvalOnDemandBSGSLazyChebyshevBatchedOnGPU(
        numMatrices, VECTOR_DIM, babyStepSize, queryCipherForClone_, MATCH_THRESHOLD, COMP_DEPTH_GPU);
