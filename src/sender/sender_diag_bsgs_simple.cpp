...
DiagonalBSGSSimpleSender::DiagonalBSGSSimpleSender(
    ...
    : DiagonalSender(ccParam, pkParam, vectorParam) 
{
  ...
  cout << "  Rotation keys: ~" << (babyStepSize - 1 + giantStepSize - 1) 
       << " (vs " << (VECTOR_DIM - 1) << " in Approach 5)" << endl;
}

// -------------------- PUBLIC FUNCTIONS --------------------

vector<Ciphertext<DCRTPoly>> DiagonalBSGSSimpleSender::computeSimilarity(
...
Ciphertext<DCRTPoly> DiagonalBSGSSimpleSender::computeSimilarityMatrixPrecompOpt(
