...
void DiagonalBSGSSimpleSenderGPU812::cacheDiagonalsOnGPU() {
	...
	cout << "[Approach 812] Cached " << allDiagonals.size() << " pre-rotated diagonals on GPU (n1=" << babyStepSize << ") in " << fixed << setprecision(2)
		 << elapsed << "s — zero homomorphic rotations" << endl;
}
...
void DiagonalBSGSSimpleSenderGPU812::cacheDiagonalsOnGPU(const vector<Ciphertext<DCRTPoly>>& preloadedDiagonals) {
	...
	cout << "[Approach 812] Caching " << preloadedDiagonals.size() << " preloaded pre-rotated diagonals on GPU..." << endl;
	auto startTime = chrono::steady_clock::now();
	...
	auto endTime   = chrono::steady_clock::now();
	double elapsed = chrono::duration<double>(endTime - startTime).count();
	cout << "[Approach 812] Cached pre-rotated diagonals (n1=" << babyStepSize << ") on GPU in " << fixed << setprecision(2) << elapsed << "s" << endl;
}
...
void DiagonalBSGSSimpleSenderGPU812::streamDiagonalsFromDisk(const string& serialDir) {
	...
	if (success) {
		gpuCacheInitialized_ = true;
		cout << "[Approach 812] Streamed " << (numMatrices * VECTOR_DIM) << " pre-rotated diagonals disk→GPU (n1=" << babyStepSize << ") in " << fixed
