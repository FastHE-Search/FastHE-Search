...
Ciphertext<DCRTPoly> OpenFHEWrapper::chebyshevCompare(CryptoContext<DCRTPoly> cc, Ciphertext<DCRTPoly> ctxt, double delta, size_t signDepth) {
	...
	const vector<double> F4_COEFS({ 0.0, 315.0 / 128.0, 0.0, -420.0 / 128.0, 0.0, 378.0 / 128.0, 0.0, -180.0 / 128.0, 0.0, 35.0 / 128.0 });
...
Ciphertext<DCRTPoly>
OpenFHEWrapper::chebyshevCompare(CryptoContext<DCRTPoly> cc, Ciphertext<DCRTPoly> ctxt, double delta, size_t signDepth, double lowerBound, double upperBound) {
	...
	cout << "[chebyshevCompare] range=[" << lowerBound << "," << upperBound << "], signDepth=" << signDepth << ", degree=" << polyDegree << " (no f4 sharpening)" << endl;
	...
	ctxt = cc->EvalChebyshevFunction([&delta](double x) -> double { return (x >= delta) ? 1 : -1; }, ctxt, lowerBound, upperBound, polyDegree);
