...
inline void print_mem_usage(const char *msg, size_t prevPeakKB = 0) {
  ...
  if (prevPeakKB > 0) {
    ...
    cout << " [delta: " << (deltaKB > 0 ? "+" : "") << (deltaKB / 1024.0)
         << " MB]";
  }
  cout << endl;
}
  ...
  if (cached < 0) {
    ...
    if (env) {
      ...
      if (err == cudaSuccess && totalMem > 0) {
        ...
        cerr << "[GPU] Warning: Could not detect GPU memory, defaulting to "
             << cached << " GB" << endl;
      }
    }
  }
...
bool serializedKeyMaterialExists() {
  return fileExists("serial/cryptocontext.bin") &&
         fileExists("serial/publickey.bin") &&
         fileExists("serial/privatekey.bin") &&
         fileExists("serial/multkey.bin") && fileExists("serial/sumkey.bin") &&
         fileExists("serial/rotkey.bin");
}

// Check if all required serialized files exist for a given approach
bool serializedDataExists(ExperimentalApproach approach, size_t numVectors,
                          size_t batchSizeHint = 16384) {
  ...
  if (approach == ExperimentalApproach::HydiaCPU ||
      approach == ExperimentalApproach::BSGSOrigCPU ||
      approach == ExperimentalApproach::BSGSPrecompCPU ||
      approach == ExperimentalApproach::BSGSPrecompOptCPU ||
      approach == ExperimentalApproach::BSGSOnlineAggCPU ||
      approach == ExperimentalApproach::HyDiaGPU ||
      approach == ExperimentalApproach::BSGSGPU ||
      approach == ExperimentalApproach::BSGSGPUPreRot) {
  ...
  } else if (approach == ExperimentalApproach::LiteratureBaseline ||
             approach == ExperimentalApproach::Grote) {
...
int main(int argc, char *argv[]) {
  ...
  size_t multDepth =
      OpenFHEWrapper::computeRequiredDepth(static_cast<size_t>(expApproach));
...
// GPU helper pointer (used for GPU approaches)
  ...
  bool loadKeysFromSerial =
      keyMaterialExists && (reuseKeysOnly || databaseExists);
  ...
  if (loadKeysFromSerial) {
    ...
    if (!Serial::DeserializeFromFile("serial/cryptocontext.bin", cc,
                                     SerType::BINARY)) {
      cerr << "Error deserializing CryptoContext" << endl;
    }
    batchSize = cc->GetEncodingParams()->GetBatchSize();

    if (!Serial::DeserializeFromFile("serial/publickey.bin", pk,
                                     SerType::BINARY)) {
      cerr << "Error deserializing public key" << endl;
    }

    if (!Serial::DeserializeFromFile("serial/privatekey.bin", sk,
                                     SerType::BINARY)) {
    ...
    if (rotKeyDeserialFile.is_open()) {
      if (!cc->DeserializeEvalAutomorphismKey(rotKeyDeserialFile,
                                              SerType::BINARY)) {
    ...
    if (expApproach == ExperimentalApproach::BSGSGPU) {
      ...
      rotationFactors.erase(
          unique(rotationFactors.begin(), rotationFactors.end()),
          rotationFactors.end());

      cout << "Approach 81 BSGS-GPU mode: generating " << rotationFactors.size()
           << " rotation keys (baby=" << (babyStepSize - 1) << ", giant=±"
           << (giantStepSize - 1) << ", evalSum=" << int(log2(batchSize)) << ")"
           << endl;
      ...
      cout << "Approach 812 BSGS-GPU-PreRot mode: generating "
           << rotationFactors.size()
           << " rotation keys (baby=" << (babyStepSize - 1) << ", giant=+"
           << (giantStepSize - 1) << ", evalSum=" << int(log2(batchSize))
           << ", NO negative keys)" << endl;
    } else if (expApproach == ExperimentalApproach::BSGSOrigCPU ||
               expApproach == ExperimentalApproach::BSGSPrecompCPU ||
               expApproach == ExperimentalApproach::BSGSPrecompOptCPU ||
               expApproach == ExperimentalApproach::BSGSOnlineAggCPU) {
      ...
      cout << "BSGS mode: generating " << rotationFactors.size()
           << " rotation keys (vs " << (VECTOR_DIM - 1) << " in standard mode)"
           << endl;
      ...
      cout << "Approach 51 HyDia-GPU mode: generating "
           << rotationFactors.size() << " rotation keys (diag 1-"
           << (VECTOR_DIM - 1) << " + binary " << VECTOR_DIM << "-"
           << (batchSize / 2) << ", no negative keys)" << endl;
    } else {
  ...
  cout << "CKKS scheme set up (depth = " << multDepth
  ...
  vector<vector<double>> plaintextVectors(numVectors,
                                          vector<double>(VECTOR_DIM));
  ...
  if (!loadDatabaseFromSerial) {
    ...
    if (expApproach == ExperimentalApproach::LiteratureBaseline ||
        expApproach == ExperimentalApproach::Grote) {
      ...
      static_cast<BlindEnroller *>(enroller)->serializeDB(plaintextVectors,
                                                          CHUNK_LEN);
      ...
      static_cast<DiagonalBSGSPreRotEnroller *>(enroller)->serializeDB(
          plaintextVectors);
    }
    delete enroller;
    {
      ...
      encDBVectorsTime =
          chrono::duration<double>(enrollEnd - enrollStart).count();
  ...
  double encQueryTime = 0, membCompTime = 0, membDecTime = 0, idxCompTime = 0,
         idxDecTime = 0;
  ...
  case ExperimentalApproach::HyDiaGPU: {
    ...
    auto dummy_ct =
        cc->Encrypt(pk, dummy_pt); // Forces RNS scaling matrix completion
    ...
    if (!gpuHelper->isReady()) {
      ...
      size_t numMatrices =
          static_cast<size_t>(ceil(double(numVectors) / double(batchSize)));
      ...
      double diagMemGB =
          (numDiagonals * bytesPerCt) / (1024.0 * 1024.0 * 1024.0);
      double keyMemGB =
          (numRotationKeys * bytesPerKey) / (1024.0 * 1024.0 * 1024.0);
      double totalMemGB = diagMemGB + keyMemGB;

      cout << "\n[HyDia-GPU] Memory estimation:" << endl;
      cout << "  Diagonals:      " << numDiagonals << " x ~15MB = " << fixed
           << setprecision(2) << diagMemGB << " GB" << endl;
      cout << "  Rotation keys:  " << numRotationKeys << " x ~45MB = " << fixed
           << setprecision(2) << keyMemGB << " GB"
           << " (DiagRot=" << (VECTOR_DIM - 1)
           << ", EvalSum=" << evalSumKeyCount << ")" << endl;
      cout << "  Total needed:   " << fixed << setprecision(2) << totalMemGB
           << " GB" << endl;
      cout << "  GPU safe limit: " << fixed << setprecision(1)
           << GPU_MEMORY_SAFE_LIMIT_GB << " GB (" << fixed << setprecision(0)
           << (GPU_MEMORY_SAFE_LIMIT_GB / GPU_TOTAL_MEMORY_GB * 100) << "% of "
           << GPU_TOTAL_MEMORY_GB << "GB)" << endl;

      if (totalMemGB > GPU_MEMORY_SAFE_LIMIT_GB) {
        cerr << "\n[HyDia-GPU] FATAL: Insufficient GPU memory for this dataset."
             << endl;
        cerr << "[HyDia-GPU] Need " << fixed << setprecision(1) << totalMemGB
             << " GB but only " << GPU_MEMORY_SAFE_LIMIT_GB
      ...
      if (!useBulkMode) {
        cout << "[HyDia-GPU] Streaming diagonals disk→GPU (default mode)"
             << endl;
...
#pragma omp parallel for num_threads(MAX_NUM_CORES) schedule(dynamic)
        for (size_t i = 0; i < numDiagonals; ++i) {
          ...
          if (Serial::DeserializeFromFile(filepath, diagonal,
                                          SerType::BINARY)) {
        ...
        cout << "[HyDia-GPU] Loaded " << numDiagonals << " diagonals in "
             << loadTime << "s" << endl;
  ...
  case ExperimentalApproach::BSGSGPU: {
    ...
    if (!gpuHelper->isReady()) {
      ...
      size_t babyKeyCount =
          static_cast<size_t>((n1_est > 1) ? (n1_est - 1) : 0);
      size_t giantKeyCount =
          static_cast<size_t>((giantStepsEst > 1) ? (giantStepsEst - 1) : 0);
      size_t evalSumKeyCount = 0;
      for (size_t shift = 1; shift < batchSize; shift *= 2)
        evalSumKeyCount++;
      ...
      cout << "  Diagonals:      " << numDiagonals << " x ~15MB = " << fixed
           << setprecision(2) << diagMemGB << " GB" << endl;
      cout << "  Rotation keys:  " << numRotationKeys << " x ~45MB = " << fixed
           << setprecision(2) << keyMemGB << " GB" << " (Baby=" << babyKeyCount
           << ", Giant=" << giantKeyCount << ", EvalSum=" << evalSumKeyCount
           << ")" << endl;
      cout << "  Total needed:   " << fixed << setprecision(2) << totalMemGB
           << " GB" << endl;
      cout << "  GPU safe limit: " << fixed << setprecision(1)
      ...
      if (totalMemGB > GPU_MEMORY_SAFE_LIMIT_GB) {
        cerr << "\n[BSGS-Simple-GPU] FATAL: Insufficient GPU memory for this "
                "dataset."
             << endl;
        cerr << "[BSGS-Simple-GPU] Need " << fixed << setprecision(1)
             << totalMemGB << " GB but only " << GPU_MEMORY_SAFE_LIMIT_GB
             << " GB safely available." << endl;
        ...
        bool useBulkMode81 =
            (streamDiagsEnv81 && std::string(streamDiagsEnv81) == "0");

        if (!useBulkMode81) {
          cout
              << "[BSGS-Simple-GPU] Streaming diagonals disk→GPU (default mode)"
              << endl;
          gpuSender->streamDiagonalsFromDisk("serial");
        } else {
...
#pragma omp parallel for num_threads(MAX_NUM_CORES) schedule(dynamic)
          for (size_t i = 0; i < numDiagonals; ++i) {
            string filepath =
                "serial/db_diagonal/index" + to_string(i) + ".bin";
            Ciphertext<DCRTPoly> diagonal;
            if (Serial::DeserializeFromFile(filepath, diagonal,
                                            SerType::BINARY)) {
              preloadedDiagonals[i] = diagonal;
          ...
          cout << "[BSGS-Simple-GPU] Loaded " << numDiagonals
               << " diagonals in " << loadTime << "s" << endl;
        ...
        cout << "[BSGS-Simple-GPU] Pre-initializing Simple BSGS rotation keys "
                "on GPU..."
             << endl;
        cout << "[BSGS-Simple-GPU] Parameters: n1=" << n1 << " (babyStepSize)"
             << endl;
        auto keyInitStart = chrono::steady_clock::now();
        gpuHelper->InitializeSimpleBSGSRotationKeysOnGPU(n1, VECTOR_DIM,
                                                         batchSize);
        auto keyInitEnd = chrono::steady_clock::now();
        double keyInitTime =
            chrono::duration<double>(keyInitEnd - keyInitStart).count();
        cout << "[BSGS-Simple-GPU] Rotation keys pre-initialized in " << fixed
             << setprecision(2) << keyInitTime << "s" << endl;
      }
    }
#else
    ...
    if (!gpuHelper->isReady()) {
      ...
      if (totalMemGB > GPU_MEMORY_SAFE_LIMIT_GB) {
        cerr << "\n[BSGS-GPU-PreRot] FATAL: Insufficient GPU memory for this "
                "dataset."
             << endl;
        cerr << "[BSGS-GPU-PreRot] Need " << fixed << setprecision(1)
             << totalMemGB << " GB but only " << GPU_MEMORY_SAFE_LIMIT_GB
             << " GB safely available." << endl;
        delete gpuHelper;
        gpuHelper = nullptr;
...
#pragma omp parallel for num_threads(MAX_NUM_CORES) schedule(dynamic)
          ...
          double loadTime =
              chrono::duration<double>(loadEnd - loadStart).count();
          cout << "[BSGS-GPU-PreRot] Loaded " << numDiagonals
               << " diagonals in " << loadTime << "s" << endl;
        ...
        int n1 = static_cast<int>(ceil(sqrt(double(VECTOR_DIM))));
        cout
            << "[BSGS-GPU-PreRot] Pre-initializing BSGS rotation keys on GPU..."
            << endl;
        cout << "[BSGS-GPU-PreRot] Parameters: n1=" << n1
             << " (babyStepSize, NO negative keys)" << endl;
        auto keyInitStart = chrono::steady_clock::now();
        gpuHelper->InitializeSimpleBSGSRotationKeysOnGPU(n1, VECTOR_DIM,
                                                         batchSize);
        auto keyInitEnd = chrono::steady_clock::now();
        double keyInitTime =
            chrono::duration<double>(keyInitEnd - keyInitStart).count();
        cout << "[BSGS-GPU-PreRot] Rotation keys pre-initialized in " << fixed
             << setprecision(2) << keyInitTime << "s" << endl;
      }
    }
#else
  ...
  cout << "[Baseline] Memory after setup: " << (baselineMemKB / 1024.0) << " MB"
       << endl;
  ...
  vector<Ciphertext<DCRTPoly>> queryCipher =
      receiver->encryptQuery(queryVector);
  ...
  if (expApproach == ExperimentalApproach::BSGSOnlineAggCPU) {
    ...
    if (diagonalReceiver != nullptr) {
      ...
      size_t numMatrices =
          static_cast<size_t>(ceil(double(numVectors) / double(batchSize)));
      double maxValue = 1.0 + (static_cast<double>(numMatrices) - 1.0) * 2.0 /
                                  sqrt(double(VECTOR_DIM));

      vector<double> scaledQuery =
          VectorUtils::plaintextNormalize(queryVector, VECTOR_DIM);
      for (size_t i = 0; i < VECTOR_DIM; i++) {
        scaledQuery[i] /= maxValue;
      }

      membershipQueryCipher = diagonalReceiver->encryptScaledQuery(scaledQuery);
      cout << "[Approach 9] Membership query scaled by 1/" << maxValue
           << " for standard depth-" << COMP_DEPTH << " comparison" << endl;
    }
  }
  ...
  expStream << queryCipher.size() << ","
            << flush; // report query communication overhead
  ...
  if (gpuHelper != nullptr &&
      (expApproach == ExperimentalApproach::BSGSGPU ||
       expApproach == ExperimentalApproach::BSGSGPUPreRot)) {
    int nBaby = static_cast<int>(ceil(sqrt(double(VECTOR_DIM))));
    cout << "[GPUHydiaHelper] Caching " << nBaby << " baby steps on GPU... "
         << flush;
    ...
    cout << "[GPUHydiaHelper] " << nBaby << " baby steps cached on GPU memory"
         << endl;
  } else if (gpuHelper != nullptr &&
             expApproach == ExperimentalApproach::HyDiaGPU) {
    cout << "[GPUHydiaHelper] Caching " << VECTOR_DIM
         << " diagonal rotations on GPU... " << flush;
    auto babyStart = chrono::steady_clock::now();
    gpuHelper->ComputeAndCacheBabyStepsOnGPU(queryCipher[0],
                                             static_cast<int>(VECTOR_DIM));
    auto babyEnd = chrono::steady_clock::now();
    double babyTime = chrono::duration<double>(babyEnd - babyStart).count();
    cout << "done (" << fixed << setprecision(2) << babyTime << "s)" << endl;
    cout << "[GPUHydiaHelper] " << VECTOR_DIM
         << " rotations cached on GPU memory" << endl;
  }
#endif
  ...
  Ciphertext<DCRTPoly> membershipCipher =
      sender->membershipScenario(membershipQueryCipher);
  end = chrono::steady_clock::now();
  duration = end - start;
  membCompTime = duration.count();
  cout << "done (wall: " << membCompTime << "s)" << endl;
  expStream << membCompTime << ","
            << flush;             // report membership computation time
  ...
  if (expApproach == ExperimentalApproach::BSGSOnlineAggCPU) {
    cout << "[Sender]\tIndex scenario skipped (Approach 9 is membership-only)"
         << endl;
    ...
    expStream << indexCipher.size() << ","
              << flush; // report index communication overhead
  ...
  // Build timing summary string (structured, parseable by benchmark_all.py)
  {
    ...
    timingSummary << "[" << timebuf << "] " << "DB=2^"
                  << static_cast<int>(std::log2(numVectors))
                  << " Approach=" << static_cast<int>(expApproach)
                  << " N=" << VECTOR_DIM << " EncQuery=" << std::fixed
                  << std::setprecision(4) << encQueryTime << "s"
                  << " MembComp=" << std::fixed << std::setprecision(4)
                  << membCompTime << "s" << " MembDec=" << std::fixed
                  << std::setprecision(4) << membDecTime << "s"
                  << " IdxComp=" << std::fixed << std::setprecision(4)
                  << idxCompTime << "s" << " IdxDec=" << std::fixed
                  << std::setprecision(4) << idxDecTime << "s"
                  << " MembResult=" << resultStatusMembership;
  ...
  if (keepSerialAfterRun) {
    ...
    try {
      ...
      cout << "Cleaned up serialized data (" << numFilesRemoved
           << " items removed from serial/)" << endl;
