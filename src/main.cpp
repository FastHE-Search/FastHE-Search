...
bool serializedKeyMaterialExists()
{
  return fileExists("serial/cryptocontext.bin") &&
         ...
         fileExists("serial/multkey.bin") &&
         fileExists("serial/sumkey.bin") &&
...
int main(int argc, char *argv[])
{
  ...
  if (approachMeta == nullptr)
  {
    cerr << "Error: approach must be from 1 to 9, or 51 (HyDia-GPU), 81 (BSGS-GPU), or 812 (BSGS-GPU-PreRot)" << endl;
    return 1;
  }
...
// GPU helper pointer (used for GPU approaches)
  ...
  // Auto-detect whether to load serialized keys and/or serialized database payloads
  ...
  {
    cout << "Found existing serialized keys, re-encrypting database for current dataset..." << endl;
  }
  else
  {
    cout << "No serialized data found, generating fresh keys and encrypting database..." << endl;
  }
  ...
  { // Else generate and serialize scheme context and keys
    ...
    if (expApproach == ExperimentalApproach::BSGSGPU)
    {
      ...
      cout << "Approach 81 BSGS-GPU mode: generating " << rotationFactors.size()
           << " rotation keys (baby=" << (babyStepSize - 1) << ", giant=±" << (giantStepSize - 1)
           << ", evalSum=" << int(log2(batchSize)) << ")" << endl;
    }
    else if (expApproach == ExperimentalApproach::BSGSGPUPreRot)
    {
      // Approach 812 (GPU BSGS with enroller pre-rotation):
      // Same as 81 but NO negative giant step keys — diagonals pre-rotated by enroller.
      size_t babyStepSize = ceil(sqrt(double(VECTOR_DIM)));
      size_t giantStepSize = ceil(double(VECTOR_DIM) / double(babyStepSize));
      ...
      cout << "Approach 812 BSGS-GPU-PreRot mode: generating " << rotationFactors.size()
           << " rotation keys (baby=" << (babyStepSize - 1) << ", giant=+" << (giantStepSize - 1)
           << ", evalSum=" << int(log2(batchSize))
             ...
             expApproach == ExperimentalApproach::BSGSOnlineAggCPU)
    {
      ...
      cout << "BSGS mode: generating " << rotationFactors.size()
           << " rotation keys (vs " << (VECTOR_DIM - 1) << " in standard mode)" << endl;
    }
    else if (expApproach == ExperimentalApproach::HyDiaGPU)
    {
      // Approach 51 (GPU Pure Diagonal): same as standard but NO negative binary keys.
      // GPU pipeline never uses negative rotations (no giant-step pre-rotation when n1=VECTOR_DIM).
      ...
      // NOTE: negative binary keys omitted — GPU EvalSum uses positive shifts only,
      // and there's no BSGS pre-rotation for pure diagonal mode.
      cout << "Approach 51 HyDia-GPU mode: generating " << rotationFactors.size()
           << " rotation keys (diag 1-" << (VECTOR_DIM - 1)
           << " + binary " << VECTOR_DIM << "-" << (batchSize / 2)
           << ", no negative keys)" << endl;
    }
    else
    {
      // Standard mode (CPU approaches): all rotation keys from 1 to VECTOR_DIM + binary ± keys
      rotationFactors.resize(VECTOR_DIM - 1);
  ...
  case ExperimentalApproach::HyDiaGPU:
  {
    ...
    {
      cerr << "[HyDia-GPU] GPU initialization failed, falling back to CPU Diagonal approach" << endl;
      delete gpuHelper;
      gpuHelper = nullptr;
      receiver = new DiagonalReceiver(cc, pk, sk, numVectors);
      sender = new DiagonalSender(cc, pk, numVectors);
    ...
    else
    {
      ...
      cout << "  GPU safe limit: " << fixed << setprecision(1) << GPU_MEMORY_SAFE_LIMIT_GB << " GB ("
           << fixed << setprecision(0) << (GPU_MEMORY_SAFE_LIMIT_GB / GPU_TOTAL_MEMORY_GB * 100) << "% of "
      ...
      if (totalMemGB > GPU_MEMORY_SAFE_LIMIT_GB)
      {
        ...
        cerr << "[HyDia-GPU] Need " << fixed << setprecision(1) << totalMemGB
             << " GB but only " << GPU_MEMORY_SAFE_LIMIT_GB << " GB safely available." << endl;
      ...
      // GPU_STREAM_DIAGS=0: opt-in to bulk-load all diagonals into CPU RAM first
      ...
      {
        cout << "[HyDia-GPU] Loading diagonals to GPU (bulk mode, GPU_STREAM_DIAGS=0)..." << endl;
...
#pragma omp parallel for num_threads(MAX_NUM_CORES) schedule(dynamic)
...
#else
    cout << "[HyDia-GPU] GPU support not compiled. Falling back to CPU Diagonal approach." << endl;
    receiver = new DiagonalReceiver(cc, pk, sk, numVectors);
    sender = new DiagonalSender(cc, pk, numVectors);
#endif
    break;
  ...
  case ExperimentalApproach::BSGSGPU:
  {
    ...
    if (!gpuHelper->isReady())
    {
      cerr << "[BSGS-Simple-GPU] GPU initialization failed, falling back to BSGS-Simple (CPU)" << endl;
      delete gpuHelper;
      gpuHelper = nullptr;
      receiver = new DiagonalReceiver(cc, pk, sk, numVectors);
    ...
    else
    {
      ...
      cout << "  Diagonals:      " << numDiagonals << " x ~15MB = " << fixed << setprecision(2) << diagMemGB << " GB" << endl;
      cout << "  Rotation keys:  " << numRotationKeys << " x ~45MB = " << fixed << setprecision(2) << keyMemGB << " GB"
           << " (Baby=" << babyKeyCount << ", Giant=" << giantKeyCount << ", EvalSum=" << evalSumKeyCount << ")" << endl;
      cout << "  Total needed:   " << fixed << setprecision(2) << totalMemGB << " GB" << endl;
      cout << "  GPU safe limit: " << fixed << setprecision(1) << GPU_MEMORY_SAFE_LIMIT_GB << " GB ("
           << fixed << setprecision(0) << (GPU_MEMORY_SAFE_LIMIT_GB / GPU_TOTAL_MEMORY_GB * 100) << "% of "
           << GPU_TOTAL_MEMORY_GB << "GB)" << endl;
      ...
      if (totalMemGB > GPU_MEMORY_SAFE_LIMIT_GB)
      {
        cerr << "\n[BSGS-Simple-GPU] FATAL: Insufficient GPU memory for this dataset." << endl;
        cerr << "[BSGS-Simple-GPU] Need " << fixed << setprecision(1) << totalMemGB
             << " GB but only " << GPU_MEMORY_SAFE_LIMIT_GB << " GB safely available." << endl;
      ...
      else
      {
        ...
        {
          cout << "[BSGS-Simple-GPU] Loading diagonals to GPU (bulk mode, GPU_STREAM_DIAGS=0)..." << endl;
          auto loadStart = chrono::steady_clock::now();
...
#pragma omp parallel for num_threads(MAX_NUM_CORES) schedule(dynamic)
        ...
        cout << "[BSGS-Simple-GPU] Pre-initializing Simple BSGS rotation keys on GPU..." << endl;
...
#else
    cout << "[BSGS-Simple-GPU] GPU support not compiled. Falling back to BSGS-Simple (CPU)." << endl;
    receiver = new DiagonalReceiver(cc, pk, sk, numVectors);
    sender = new DiagonalBSGSSimpleSender(cc, pk, numVectors);
#endif
    break;
  ...
  case ExperimentalApproach::BSGSGPUPreRot:
  {
    ...
    cout << "[BSGS-GPU-PreRot] Initializing GPU BSGS sender (enroller pre-rotation)..." << endl;
    ...
    if (!gpuHelper->isReady())
    {
      cerr << "[BSGS-GPU-PreRot] GPU initialization failed, falling back to BSGS-Simple (CPU)" << endl;
      delete gpuHelper;
      gpuHelper = nullptr;
      receiver = new DiagonalReceiver(cc, pk, sk, numVectors);
      sender = new DiagonalBSGSSimpleSender(cc, pk, numVectors);
    ...
    else
    {
      ...
      cout << "  Diagonals:      " << numDiagonals << " x ~15MB = " << fixed << setprecision(2) << diagMemGB << " GB" << endl;
      cout << "  Rotation keys:  " << numRotationKeys << " x ~45MB = " << fixed << setprecision(2) << keyMemGB << " GB"
           << " (Baby=" << babyKeyCount << ", Giant=+" << giantKeyCount
           << ", EvalSum=" << evalSumKeyCount << ", NO negative keys)" << endl;
      cout << "  Total needed:   " << fixed << setprecision(2) << totalMemGB << " GB" << endl;
      cout << "  GPU safe limit: " << fixed << setprecision(1) << GPU_MEMORY_SAFE_LIMIT_GB << " GB ("
           << fixed << setprecision(0) << (GPU_MEMORY_SAFE_LIMIT_GB / GPU_TOTAL_MEMORY_GB * 100) << "% of "
           << GPU_TOTAL_MEMORY_GB << "GB)" << endl;
      ...
      if (totalMemGB > GPU_MEMORY_SAFE_LIMIT_GB)
      {
        cerr << "\n[BSGS-GPU-PreRot] FATAL: Insufficient GPU memory for this dataset." << endl;
        cerr << "[BSGS-GPU-PreRot] Need " << fixed << setprecision(1) << totalMemGB
             << " GB but only " << GPU_MEMORY_SAFE_LIMIT_GB << " GB safely available." << endl;
        delete gpuHelper;
        gpuHelper = nullptr;
        return 1;
      ...
      {
        // Initialize GPU sender (uses CachePreRotatedDiagonalsOnGPU — no rotation)
        receiver = new DiagonalReceiver(cc, pk, sk, numVectors);
        ...
        if (!useBulkMode812)
        {
          cout << "[BSGS-GPU-PreRot] Streaming pre-rotated diagonals disk→GPU (default mode)" << endl;
          gpuSender->streamDiagonalsFromDisk("serial");
        }
        else
        {
          cout << "[BSGS-GPU-PreRot] Loading pre-rotated diagonals to GPU (bulk mode)..." << endl;
          auto loadStart = chrono::steady_clock::now();
...
#pragma omp parallel for num_threads(MAX_NUM_CORES) schedule(dynamic)
        ...
        // Pre-initialize BSGS rotation keys on GPU (same as 81, but no negative keys)
        int n1 = static_cast<int>(ceil(sqrt(double(VECTOR_DIM))));
...
#else
    cout << "[BSGS-GPU-PreRot] GPU support not compiled. Falling back to BSGS-Simple (CPU)." << endl;
    receiver = new DiagonalReceiver(cc, pk, sk, numVectors);
    sender = new DiagonalBSGSSimpleSender(cc, pk, numVectors);
#endif
    break;
  ...
  if (expApproach == ExperimentalApproach::BSGSOnlineAggCPU)
  {
    ...
    if (diagonalReceiver != nullptr)
    {
      ...
      cout << "[Approach 9] Membership query scaled by 1/" << maxValue << " for standard depth-"
           << COMP_DEPTH << " comparison" << endl;
    }
  }
...
#ifdef USE_GPU_ACCELERATION
  // Pre-cache baby-step rotations OUTSIDE membership timing window for fair comparison.
  // Approach 81 (BSGS-Simple-GPU): cache only n1 baby steps.
  ...
  // Build timing summary string (structured, parseable by benchmark_all.py)
  {
    ...
    timingSummary << "[" << timebuf << "] "
                  << "DB=2^" << static_cast<int>(std::log2(numVectors))
                  << " Approach=" << static_cast<int>(expApproach)
                  << " N=" << VECTOR_DIM
                  << " EncQuery=" << std::fixed << std::setprecision(4) << encQueryTime << "s"
                  << " MembComp=" << std::fixed << std::setprecision(4) << membCompTime << "s"
                  << " MembDec=" << std::fixed << std::setprecision(4) << membDecTime << "s"
                  << " IdxComp=" << std::fixed << std::setprecision(4) << idxCompTime << "s"
                  << " IdxDec=" << std::fixed << std::setprecision(4) << idxDecTime << "s"
                  << " MembResult=" << resultStatusMembership;

    cout << endl
         << "\t" << timingSummary.str() << endl;
  }
  ...
  // Clean up serialized data after successful completion (match old repo behavior)
  ...
  cout << endl
       << "\tProgram successfully terminated" << endl;
  return 0;
}