...
static bool serializedDataExists(ExperimentalApproach approach,
                                 size_t numVectors,
                                 size_t batchSizeHint = 16384) {
  if (!fileExists("serial/cryptocontext.bin") ||
  ...
  case ExperimentalApproach::BSGSPrecompOptCPU:
      ...
      return false;
    {
      ...
      if (!fileExists("serial/db_diagonal/index" + std::to_string(lastIdx) +
                      ".bin"))
        return false;
    }
    break;
...
static bool isGPUApproach(ExperimentalApproach approach) {
  return approach == ExperimentalApproach::HyDiaGPU ||
...
static size_t computeDepthWithOverride(ExperimentalApproach approach,
                                       size_t compDepth) {
  size_t depth = 0;
  switch (approach) {
  case ExperimentalApproach::LiteratureBaseline:
    ...
    break;
  case ExperimentalApproach::Grote:
    depth = 1 + 2 + ALPHA_DEPTH + 3 + compDepth;
    break;
  case ExperimentalApproach::BlindMatch:
    depth = 1 + 1 + compDepth;
    break;
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 4) {
    ...
    cerr << "    51/81/812 = GPU (FRGC2 dataset does not fit in GPU memory)"
         << endl;
    cerr << endl;
  ...
  if (argc > 5) {
    ...
    if (compDepth == 0) {
      cerr << "Error: comp_depth must be a positive integer, got: " << argv[5]
           << endl;
      return 1;
    }
    cout << "Comparison depth override: " << compDepth
         << " (compile-time default was " << COMP_DEPTH << ")" << endl;
  }

  if (queryStart > queryEnd || queryEnd >= 50) {
    cerr << "Error: invalid query range [" << queryStart << ", " << queryEnd
         << "]. Must be within [0, 49]." << endl;
    return 1;
  }
  ...
  if (isGPUApproach(expApproach)) {
    cerr << "Error: GPU approach " << approachInt << " ("
         << approachMeta->display
         << ") is not supported for FRGC2 accuracy testing." << endl;
  ...
  if (isUnsupportedForAccuracy(expApproach)) {
    cerr << "Error: Approach " << approachInt << " (" << approachMeta->display
         << ") is membership-only and does not support the index scenario"
         << endl;
    cerr << "required for per-vector accuracy evaluation. Use approaches 1-8."
         << endl;
    return 1;
  }
  ...
  cout << "Query range: [" << queryStart << ", " << queryEnd << "] ("
       << (queryEnd - queryStart + 1) << " queries)" << endl;
  cout << "Experimental approach: " << approachMeta->display
       << " (ID=" << approachInt << ")" << endl;
  cout << "FRGC2 data directory: " << frgc2Dir << endl;
  cout << "Comparison depth: " << compDepth
       << "  Total multiplicative depth: " << multDepth << endl;
  ...
  if (!fileStream.is_open()) {
    ...
    cerr << "Please ensure the FRGC2 accuracy test files exist in: " << frgc2Dir
         << endl;
    return 1;
  }
  ...
  if (loadFromSerial) {
    ...
    if (expApproach == ExperimentalApproach::BSGSOrigCPU ||
  ...
  cout << "CKKS scheme set up (depth = " << multDepth
       << ", batch size = " << batchSize << ", scaling factor = " << scalFactor
  ...
  if (!loadFromSerial) {
    ...
    if (enroller == nullptr) {
      cerr << "Error: unsupported approach for enrollment in accuracy binary"
           << endl;
      return 1;
    }
    delete enroller;
  ...
  default:
    cerr << "Error: unsupported approach for accuracy binary" << endl;
    return 1;
  }

  // ==================== Open Log File ====================
  std::string logFileName = "accuracy_approach" + std::to_string(approachInt) +
                            "_depth" + std::to_string(compDepth) + "_q" +
  ...
  logFile << "queryIndex,querySubjectID,approach,depth,comp_depth,scale,"
             "numDBVectors,"
          << "enc_TP,enc_FN,enc_TN,enc_FP,"
          << "plain_TP,plain_FN,plain_TN,plain_FP,"
          << "encQueryTime_s,idxCompTime_s,idxDecTime_s" << endl;
  ...
  for (size_t queryIndex = queryStart; queryIndex <= queryEnd; ++queryIndex) {
    ...
    vector<double> boolVec =
        OpenFHEWrapper::decryptVectorToVector(cc, sk, indexCipher);
    ...
    for (size_t i = 0; i < numVectors; i++) {
      ...
      plaintextPositive = (VectorUtils::plaintextCosineSim(
                               queryVector[queryIndex], plaintextVectors[i]) >=
                           MATCH_THRESHOLD);
    ...
    cout << "Encrypted  TP=" << tp << " FN=" << fn << " TN=" << tn
         << " FP=" << fp << endl;
    cout << "Plaintext  TP=" << tpPlain << " FN=" << fnPlain
         << " TN=" << tnPlain << " FP=" << fpPlain << endl;

    // Write per-query CSV row
    logFile << queryIndex << "," << queryID[queryIndex] << "," << approachInt
            << "," << multDepth << "," << compDepth << "," << scalFactor << ","
            << numVectors << "," << tp << "," << fn << "," << tn << "," << fp
            << "," << tpPlain << "," << fnPlain << "," << tnPlain << ","
            << fpPlain << "," << fixed << setprecision(4) << encQueryTime << ","
            << idxCompTime << "," << idxDecTime << endl;
  ...
  cout << endl
       << "========== Aggregate Results (" << numQueries
       << " queries) ==========" << endl;
  cout << "Approach: " << approachMeta->display << " (ID=" << approachInt
       << ")  Depth: " << multDepth << "  Comp_depth: " << compDepth
       << "  Scale: " << scalFactor << endl;
  cout << "Encrypted  Total TP=" << totalTP << " FN=" << totalFN
       << " TN=" << totalTN << " FP=" << totalFP << endl;
  cout << "Plaintext  Total TP=" << totalTPPlain << " FN=" << totalFNPlain
       << " TN=" << totalTNPlain << " FP=" << totalFPPlain << endl;

  double precision = (totalTP + totalFP > 0)
                         ? double(totalTP) / double(totalTP + totalFP)
                         : 0.0;
  double recall = (totalTP + totalFN > 0)
                      ? double(totalTP) / double(totalTP + totalFN)
                      : 0.0;
  double f1 = (precision + recall > 0)
                  ? 2.0 * precision * recall / (precision + recall)
                  : 0.0;
  double accuracy = (totalTP + totalTN + totalFP + totalFN > 0)
                        ? double(totalTP + totalTN) /
                              double(totalTP + totalTN + totalFP + totalFN)
                        : 0.0;

  double precisionPlain =
      (totalTPPlain + totalFPPlain > 0)
          ? double(totalTPPlain) / double(totalTPPlain + totalFPPlain)
          : 0.0;
  double recallPlain =
      (totalTPPlain + totalFNPlain > 0)
          ? double(totalTPPlain) / double(totalTPPlain + totalFNPlain)
          : 0.0;
  double f1Plain =
      (precisionPlain + recallPlain > 0)
          ? 2.0 * precisionPlain * recallPlain / (precisionPlain + recallPlain)
          : 0.0;
  double accuracyPlain =
      (totalTPPlain + totalTNPlain + totalFPPlain + totalFNPlain > 0)
          ? double(totalTPPlain + totalTNPlain) /
                double(totalTPPlain + totalTNPlain + totalFPPlain +
                       totalFNPlain)
          : 0.0;

  cout << "Encrypted  Precision=" << fixed << setprecision(6) << precision
       << "  Recall=" << recall << "  F1=" << f1 << "  Accuracy=" << accuracy
       << endl;
  cout << "Plaintext  Precision=" << fixed << setprecision(6) << precisionPlain
       << "  Recall=" << recallPlain << "  F1=" << f1Plain
       << "  Accuracy=" << accuracyPlain << endl;

  double amortizedIdxTime = totalIdxCompTime / numQueries;
  cout << "Total index scenario time=" << fixed << setprecision(4)
       << totalIdxCompTime << "s  Amortized=" << amortizedIdxTime << "s/query"
       << endl;
  ...
  try {
    ...
    std::cerr << "Warning: Could not remove serial/ folder: " << e.what()
              << endl;
  }
