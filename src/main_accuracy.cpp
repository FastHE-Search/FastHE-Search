...
// This mirrors OpenFHEWrapper::computeRequiredDepth() but uses the caller-supplied
// compDepth instead of the compile-time COMP_DEPTH constant.
...
int main(int argc, char *argv[])
{
  ...
  if (argc < 4)
  {
    ...
    cerr << "  frgc2_dir  : path to directory containing FRGC2 files (default: ../test)" << endl;
  ...
  {
    cerr << "Error: GPU approach " << approachInt << " (" << approachMeta->display
         << ") is not supported for FRGC2 accuracy testing." << endl;
    cerr << "The FRGC2 dataset is too large to fit in GPU memory for this workload." << endl;
  ...
  if (isUnsupportedForAccuracy(expApproach))
  {
    cerr << "Error: Approach " << approachInt << " (" << approachMeta->display
         << ") is membership-only and does not support the index scenario" << endl;
  ...
  cout << "CKKS scheme set up (depth = " << multDepth << ", batch size = " << batchSize
       << ", scaling factor = " << scalFactor << ", comp_depth = " << compDepth << ")" << endl;
  ...
  // ==================== Open Log File ====================
  std::string logFileName = "accuracy_approach" + std::to_string(approachInt) + "_depth" + std::to_string(compDepth) + "_q" + std::to_string(queryStart) + "-" + std::to_string(queryEnd) + ".csv";
  ...
  logFile << "queryIndex,querySubjectID,approach,depth,comp_depth,scale,numDBVectors,"
  ...
  for (size_t queryIndex = queryStart; queryIndex <= queryEnd; ++queryIndex)
  {
    ...
    logFile << queryIndex << "," << queryID[queryIndex] << ","
            << approachInt << "," << multDepth << "," << compDepth << "," << scalFactor << "," << numVectors << ","
            << tp << "," << fn << "," << tn << "," << fp << ","
            << tpPlain << "," << fnPlain << "," << tnPlain << "," << fpPlain << ","
  ...
  cout << endl
       << "========== Aggregate Results (" << numQueries << " queries) ==========" << endl;
  ...
  double accuracy = (totalTP + totalTN + totalFP + totalFN > 0)
                        ? double(totalTP + totalTN) / double(totalTP + totalTN + totalFP + totalFN)
                        : 0.0;
  ...
  cout << "Total index scenario time=" << fixed << setprecision(4) << totalIdxCompTime
       << "s  Amortized=" << amortizedIdxTime << "s/query" << endl;
