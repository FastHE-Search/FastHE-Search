...
static std::vector<double> ComputeChebyshevCoefficients(
    std::function<double(double)> func, double a, double b, uint32_t degree)
{
...
GPUHydiaHelper::GPUHydiaHelper(CryptoContext<DCRTPoly> cc,
                               const KeyPair<DCRTPoly> &keys,
                               const std::vector<int> &gpuDevices,
                               int batchSize,
                               double gpuSafeMemoryLimitGB)
    : cc_(cc), keys_(keys), gpuContextReady_(false), batchSize_(batchSize),
      diagonalsCached_(false), babyStepsCached_(false),
      numCachedDiagonals_(0), numCachedGroups_(0), perGroupCaching_(false),
      diagonalsPreRotated_(false), preRotationN1_(0)
{
    ...
    std::cout << "[GPUHydiaHelper] GPU safe memory limit: " << std::fixed << std::setprecision(2)
              << gpuSafeMemoryLimitGB_ << " GB" << std::endl;
    ...
    {
        std::cerr << "[GPUHydiaHelper] ✗ GPU initialization failed: "
                  << e.what() << std::endl;
...
void GPUHydiaHelper::CacheDiagonalsWithPreRotationOnGPU(
    const std::vector<Ciphertext<DCRTPoly>> &diagonals,
    size_t numMatrices, size_t vectorDim, int n1)
{
    ...
    {
        std::cerr << "[GPUHydiaHelper] Diagonal count mismatch: got " << diagonals.size()
                  << " expected " << totalDiags << std::endl;
        return;
    }
    ...
    try
    {
        std::cout << "[GPUHydiaHelper] Caching " << totalDiags << " diagonals with pre-rotation (n1="
                  << n1 << ") on GPU..." << std::flush;
        ...
        std::cout << "[GPUHydiaHelper] ✓ " << cachedGPUDiagonals_.size()
                  << " diagonals pre-rotated and cached on GPU" << std::endl;
        ...
        {
            std::cout << "[GPUHydiaHelper] Freed " << preRotKeysFreed
                      << " pre-rotation keys (no longer needed)" << std::endl;
        }
    }
    catch (const std::exception &e)
    {
...
void GPUHydiaHelper::CachePreRotatedDiagonalsOnGPU(
    const std::vector<Ciphertext<DCRTPoly>> &diagonals,
    size_t numMatrices, size_t vectorDim, int n1)
{
    ...
    try
    {
        std::cout << "[GPUHydiaHelper] Uploading " << totalDiags
                  << " pre-rotated diagonals to GPU (no server-side rotation, n1="
                  << n1 << ")..." << std::flush;
        auto startTime = std::chrono::steady_clock::now();
        ...
        std::cout << " done (" << std::fixed << std::setprecision(2) << elapsed << "s)" << std::endl;
        std::cout << "[GPUHydiaHelper] ✓ " << cachedGPUDiagonals_.size()
                  << " pre-rotated diagonals uploaded to GPU (zero homomorphic rotations)" << std::endl;
    }
    catch (const std::exception &e)
    {
...
bool GPUHydiaHelper::StreamPreRotatedDiagonalsToGPU(const std::string &serialDir,
                                                    size_t numDiagonals, size_t numMatrices, int n1)
{
    if (!gpuContextReady_)
    {
    ...
    std::cout << "[GPUHydiaHelper] Streaming " << totalDiagonals
              << " pre-rotated diagonals to GPU (no server-side rotation)..." << std::endl;
    ...
    try
    {
        ...
        std::cout << "[GPUHydiaHelper] OMP-parallel streaming: batch=" << batchSize
                  << ", threads=" << ompThreads << " (no rotation)" << std::endl;
...
#pragma omp parallel for num_threads(ompThreads) schedule(dynamic)
            ...
            std::cout << "\r[GPUHydiaHelper] Streamed " << loaded << "/" << totalDiagonals
                      << " diagonals (" << std::fixed << std::setprecision(1)
        ...
        std::cout << "[GPUHydiaHelper] ✓ Streamed " << loaded << " pre-rotated diagonals to GPU in "
...
void GPUHydiaHelper::ComputeAndCacheBabyStepsOnGPU(
    const Ciphertext<DCRTPoly> &queryVector,
    int n1)
{
    ...
    try
    {
        std::cout << "[GPUHydiaHelper] Computing " << n1
                  << " baby steps directly on GPU..." << std::flush;
        ...
        {
            if ((i & (i - 1)) != 0)
            { // Not a power of 2 → not an EvalSum key
        ...
        if (babyKeysFreed > 0)
        {
            std::cout << "[GPUHydiaHelper] Freed " << babyKeysFreed
                      << " baby-step rotation keys (no longer needed, kept "
...
void GPUHydiaHelper::CacheBabyStepsOnGPU(const std::vector<Ciphertext<DCRTPoly>> &babySteps)
{
    ...
    try
    {
        ...
        babyStepsCached_ = true;
        std::cout << " done (" << std::fixed << std::setprecision(2) << elapsed << "s)" << std::endl;
        std::cout << "[GPUHydiaHelper] ✓ " << cachedGPUBabySteps_.size()
                  << " baby steps cached on GPU memory" << std::endl;
    }
    catch (const std::exception &e)
    {
...
void GPUHydiaHelper::InitializeSimpleBSGSRotationKeysOnGPU(int n1, int vectorDim, size_t numSlots)
{
    ...
    try
    {
        ...
        std::cout << "  Giant: " << giantKeysInit << ", EvalSum: " << evalSumKeysInit
                  << " (baby-step keys skipped — computed on CPU)" << std::endl;
        std::cout << "  Total time: " << std::fixed << std::setprecision(2) << elapsed << "s" << std::endl;
    }
    catch (const std::exception &e)
    {
        std::cerr << "[GPUHydiaHelper] Failed to initialize Simple BSGS rotation keys: "
                  << e.what() << std::endl;
        throw;
    }
}
...
void GPUHydiaHelper::EnsureMatrixStreams(size_t numNeeded)
{
    ...
    try
    {
        ...
        size_t memLimitedStreams = (availableForStreams > 0 && perMatrixBytes > 0)
                                       ? availableForStreams / perMatrixBytes
                                       : 2;
        ...
        if (envStreams)
        {
            ...
            if (userStreams >= 1 && userStreams <= 64)
            {
                ...
                std::cout << "[GPUHydiaHelper] Using GPU_NUM_STREAMS=" << target
        ...
        for (size_t i = 0; i < target; ++i)
        {
            ...
            if (err != cudaSuccess)
            {
                std::cerr << "[GPUHydiaHelper] Failed to create CUDA stream " << i
        ...
        std::cout << "[GPUHydiaHelper] ✓ CUDA streams: " << target << " created on-demand"
                  << " (needed=" << numNeeded
                  << ", GPU: " << prop.name
                  << ", SMs: " << prop.multiProcessorCount
                  << ", free: " << (freeMem / (1024 * 1024)) << " MB"
                  << ", ~" << (perMatrixBytes / (1024 * 1024)) << " MB/concurrent matrix)" << std::endl;
    }
    catch (const std::exception &e)
    {
        std::cerr << "[GPUHydiaHelper] Warning: Failed to create CUDA streams: "
                  << e.what() << std::endl;
        streamsInitialized_ = false;
        numMatrixStreams_ = 0;
    }
...
bool GPUHydiaHelper::StreamDiagonalsToGPU(const std::string &serialDir,
                                          size_t numDiagonals, size_t numMatrices, int n1)
{
    if (!gpuContextReady_)
    {
...
#pragma omp parallel for num_threads(ompThreads) schedule(dynamic)
            ...
            for (size_t i = 0; i < currentBatch; ++i)
            {
                ...
                if (!loadOk[i])
                {
                    if (errors < 5)
                    {
                        std::cerr << "[GPUHydiaHelper] Error loading diagonal flatIdx=" << flatIdx
                                  << " path=" << serialDir << "/db_diagonal/index" << flatIdx << ".bin" << std::endl;
                    }
                    errors++;
                    cachedGPUDiagonals_.push_back(nullptr);
                    continue;
        ...
        std::cout << "[GPUHydiaHelper] ✓ Streamed " << loaded << " diagonals to GPU in "
                  << std::fixed << std::setprecision(2) << elapsed << "s" << std::endl;
        std::cout << "[GPUHydiaHelper] Peak CPU RAM: ~" << std::fixed << std::setprecision(1)
                  << (batchSize * ctSizeBytes / (1024.0 * 1024.0 * 1024.0)) << " GB ("
                  << batchSize << " diags × " << (ctSizeBytes / (1024 * 1024)) << " MB)  vs bulk-load ~"
        ...
        // === Dead key cleanup: free pre-rotation keys no longer needed ===
        {
            ...
            {
                std::cout << "[GPUHydiaHelper] Freed " << preRotKeysFreed
                          << " pre-rotation keys (negative indices) — no longer needed" << std::endl;
            }
        }
...
void *GPUHydiaHelper::ProcessMatrixOnStream(
    size_t matrixIdx, size_t vectorDim, int n1,
    int numGiantSteps, double delta, size_t signDepth,
    cudaStream_t stream)
{
...
void *GPUHydiaHelper::ProcessSimilarityMatrixOnStream(
    size_t matrixIdx, size_t vectorDim,
    double delta, size_t signDepth,
    cudaStream_t stream)
{
    ...
    // Apply Chebyshev comparison — stays on GPU
    void *chebResult = EvalChebyshevCompareOnGPU(
        static_cast<void *>(gpuAccumulator), delta, signDepth);
...
Ciphertext<DCRTPoly> GPUHydiaHelper::EvalOnDemandBSGSLazyChebyshevAndAggregateOnGPU(
    size_t numMatrices, size_t vectorDim, int n1,
    const Ciphertext<DCRTPoly> &templateCt,
    double delta, size_t signDepth, size_t numSlots)
{
    ...
    {
        throw std::runtime_error("[GPUHydiaHelper] n1 mismatch: diagonals pre-rotated with n1=" +
                                 std::to_string(preRotationN1_) + " but query uses n1=" + std::to_string(n1));
    }
    ...
    try
    {
        ...
        size_t effectiveStreams = streamsInitialized_
        ...
        if (effectiveStreams > 1)
        {
            ...
            std::cout << "[GPUHydiaHelper] 🚀 A81 Multi-stream: " << effectiveStreams
            ...
            for (size_t waveStart = 0; waveStart < numMatrices; waveStart += effectiveStreams)
            {
                ...
                std::cout << "[CUDA Streams] A81-Membership Wave " << waveNum << "/" << totalWaves
        ...
        else
        {
            ...
            std::cout << "[GPUHydiaHelper] ⚠ A81 Single-stream mode (numMatrices=" << numMatrices
        ...
        std::cout << "[GPUHydiaHelper] A81 BSGS+Chebyshev: " << numMatrices << " matrices in "
                  << std::fixed << std::setprecision(3) << phase1Time << "s"
                  << " (" << effectiveStreams << " streams, ~"
                  << std::setprecision(1) << (phase1Time / numMatrices * 1000.0) << " ms/matrix)" << std::endl;
    ...
    {
        std::cerr << "[GPUHydiaHelper] EvalOnDemandBSGSLazyChebyshevAndAggregateOnGPU failed: "
                  << e.what() << std::endl;
        throw;
    }
}

std::vector<Ciphertext<DCRTPoly>> GPUHydiaHelper::EvalOnDemandBSGSLazyChebyshevBatchedOnGPU(
    size_t numMatrices, size_t vectorDim, int n1,
    const Ciphertext<DCRTPoly> &templateCt,
    double delta, size_t signDepth)
{
    ...
    try
    {
        ...
        std::cout << "[GPUHydiaHelper] A81 Index BSGS+Cheb (" << numMatrices << " matrices): "
                  << std::fixed << std::setprecision(3) << phase1Time << "s"
                  << " (" << effectiveStreams << " streams, ~"
                  << std::setprecision(1) << (phase1Time / numMatrices * 1000.0) << " ms/matrix)" << std::endl;
    ...
    {
        std::cerr << "[GPUHydiaHelper] EvalOnDemandBSGSLazyChebyshevBatchedOnGPU failed: "
                  << e.what() << std::endl;
        throw;
    }
}
...
void *GPUHydiaHelper::EvalChebyshevCompareOnGPU(void *gpuCtVoid, double delta, size_t signDepth)
{
    ...
    std::vector<double> chebyCoeffs = ComputeChebyshevCoefficients(
        [&delta](double x) -> double
        { return (x >= delta) ? 1.0 : -1.0; },
        -1.0, 1.0, polyDegree);
...
Ciphertext<DCRTPoly> GPUHydiaHelper::EvalSimilarityChebyshevAndAggregateOnGPU(
    size_t numMatrices, size_t vectorDim,
    const Ciphertext<DCRTPoly> &templateCt,
    double delta, size_t signDepth, size_t numSlots)
{
    ...
    {
        std::cerr << "[GPUHydiaHelper] EvalSimilarityChebyshevAndAggregateOnGPU PHASE 0failed: "
                  << e.what() << std::endl;
        throw;
    }
    ...
    try
    {
        ...
        {
            size_t totalWaves = (numMatrices + effectiveStreams - 1) / effectiveStreams;
            std::cout << "[GPUHydiaHelper] 🚀 A51 Multi-stream: " << effectiveStreams
                      << " concurrent streams for " << numMatrices << " matrices"
                      << " (" << totalWaves << " wave" << (totalWaves > 1 ? "s" : "") << ")" << std::endl;
            std::cout << "[CUDA Streams] batchSize=" << cc_->GetEncodingParams()->GetBatchSize()
                      << ", numCKKSBatches=" << numMatrices
            ...
            for (size_t waveStart = 0; waveStart < numMatrices; waveStart += effectiveStreams)
            {
                ...
                std::cout << "[CUDA Streams] A51-Membership Wave " << waveNum << "/" << totalWaves
                          << ": matrices [" << waveStart << "-" << (waveEnd - 1) << "]"
                          << " on " << waveSize << "/" << numMatrixStreams_ << " streams"
                          << (waveSize < numMatrixStreams_ ? " (UNDERUTILIZED)" : " (FULL)")
                          << std::endl;
                ...
                for (size_t w = 0; w < waveSize; ++w)
                {
                    ...
                    cudaStream_t stream = matrixStreams_[w % numMatrixStreams_];
                    gpuChebResults[m] = static_cast<FIDESlib::CKKS::Ciphertext *>(
                        ProcessSimilarityMatrixOnStream(
                            m, vectorDim, delta, signDepth, stream));
                }
        ...
        {
            std::cout << "[GPUHydiaHelper] ⚠ A51 Single-stream mode (numMatrices=" << numMatrices
                      << ") — no CUDA stream parallelism" << std::endl;
            for (size_t m = 0; m < numMatrices; ++m)
            {
                gpuChebResults[m] = static_cast<FIDESlib::CKKS::Ciphertext *>(
                    ProcessSimilarityMatrixOnStream(
                        m, vectorDim, delta, signDepth, nullptr));
            }
            cudaDeviceSynchronize();
        }
        ...
        std::cout << "[GPUHydiaHelper] A51 Similarity+Chebyshev: " << numMatrices
                  << " matrices in " << std::fixed << std::setprecision(3) << simTime << "s"
                  << " (" << effectiveStreams << " streams, ~"
                  << std::setprecision(1) << (simTime / numMatrices * 1000.0) << " ms/matrix)" << std::endl;
    }
    catch (const std::exception &e)
    {
        std::cerr << "[GPUHydiaHelper] EvalSimilarityChebyshevAndAggregateOnGPU PHASE 1 failed: "
                  << e.what() << std::endl;
        throw;
    }
    ...
    {
        std::cerr << "[GPUHydiaHelper] EvalSimilarityChebyshevAndAggregateOnGPU PHASE 2 failed: "
                  << e.what() << std::endl;
        throw;
    }
    ...
    try
    {
        ...
        std::cout << "[GPUHydiaHelper] A51 EvalSum (" << static_cast<int>(std::log2(numSlots))
                  << " rotations): " << std::fixed << std::setprecision(3) << sumTime << "s" << std::endl;
        ...
        std::cout << "[GPUHydiaHelper] A51 Membership total: " << std::fixed << std::setprecision(2)
                  << totalTime << "s (Sim+Cheb=" << simTime << "s, Add=" << addTime
                  << "s, Sum=" << sumTime << "s)" << std::endl;
    ...
    {
        std::cerr << "[GPUHydiaHelper] EvalSimilarityChebyshevAndAggregateOnGPU PHASE 4 failed: "
                  << e.what() << std::endl;
        throw;
    }
}
...
std::vector<Ciphertext<DCRTPoly>> GPUHydiaHelper::EvalSimilarityAndChebyshevBatchedOnGPU(
    size_t numMatrices, size_t vectorDim,
    const Ciphertext<DCRTPoly> &templateCt,
    double delta, size_t signDepth)
{
    ...
    try
    {
        ...
        {
            size_t totalWaves = (numMatrices + effectiveStreams - 1) / effectiveStreams;
            std::cout << "[GPUHydiaHelper] 🚀 A51 Index Multi-stream: " << effectiveStreams
                      << " concurrent streams for " << numMatrices << " matrices"
                      << " (" << totalWaves << " wave" << (totalWaves > 1 ? "s" : "") << ")" << std::endl;
            std::cout << "[CUDA Streams] batchSize=" << cc_->GetEncodingParams()->GetBatchSize()
                      << ", numCKKSBatches=" << numMatrices
            ...
            for (size_t waveStart = 0; waveStart < numMatrices; waveStart += effectiveStreams)
            {
                ...
                std::cout << "[CUDA Streams] A51-Index Wave " << waveNum << "/" << totalWaves
                          << ": matrices [" << waveStart << "-" << (waveEnd - 1) << "]"
                          << " on " << waveSize << "/" << numMatrixStreams_ << " streams"
                          << (waveSize < numMatrixStreams_ ? " (UNDERUTILIZED)" : " (FULL)")
                          << std::endl;
        ...
        {
            std::cout << "[GPUHydiaHelper] ⚠ A51 Index Single-stream mode (numMatrices=" << numMatrices
                      << ") — no CUDA stream parallelism" << std::endl;
            for (size_t m = 0; m < numMatrices; ++m)
            {
                gpuChebResults[m] = static_cast<FIDESlib::CKKS::Ciphertext *>(
        ...
        std::cout << "[GPUHydiaHelper] A51 Batched SimCheb: " << numMatrices << " matrices in "
                  << std::fixed << std::setprecision(3) << phase1Time << "s"
                  << " (" << effectiveStreams << " streams, ~"
                  << std::setprecision(1) << (phase1Time / numMatrices * 1000.0) << " ms/matrix)" << std::endl;
        ...
        std::cout << "[GPUHydiaHelper] A51 Batched Download (" << numMatrices << " cts): "
                  << std::fixed << std::setprecision(3) << phase2Time << "s" << std::endl;

        auto endTime = std::chrono::steady_clock::now();
        double totalTime = std::chrono::duration<double>(endTime - startTime).count();
        std::cout << "[GPUHydiaHelper] A51 Batched Index total: "
                  << std::fixed << std::setprecision(3) << totalTime << "s" << std::endl;
    ...
    {
        std::cerr << "[GPUHydiaHelper] EvalSimilarityAndChebyshevBatchedOnGPU failed: "
                  << e.what() << std::endl;
        throw;
    }
}
