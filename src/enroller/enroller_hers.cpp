...
HersEnroller::HersEnroller(CryptoContext<DCRTPoly> ccParam, PublicKey<DCRTPoly> pkParam, size_t vectorParam)
: cc(ccParam), pk(pkParam), numVectors(vectorParam) {
}

// -------------------- PUBLIC FUNCTIONS --------------------

vector<vector<Ciphertext<DCRTPoly>>> HersEnroller::encryptDB(vector<vector<double>>& database) {
