...
BlindEnroller::BlindEnroller(CryptoContext<DCRTPoly> ccParam, PublicKey<DCRTPoly> pkParam, size_t vectorParam) : HersEnroller(ccParam, pkParam, vectorParam) {
}
...
void BlindEnroller::serializeDBThread(vector<vector<double>>& database, size_t chunkLength, size_t matrix, size_t index) {
	...
	for (size_t i = 0; i < chunksPerBatch && i < remainingVectors; i++) {
		...
		copy(database[i + matrix * chunksPerBatch].begin() + index, database[i + matrix * chunksPerBatch].begin() + index + chunkLength, currentVector.begin() + i * chunkLength);
	}
