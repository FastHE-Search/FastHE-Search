...
DiagonalBSGSPreRotEnroller::DiagonalBSGSPreRotEnroller(
    CryptoContext<DCRTPoly> ccParam,
    PublicKey<DCRTPoly> pkParam,
    size_t vectorParam)
  : DiagonalEnroller(ccParam, pkParam, vectorParam),
    babyStepSize_(static_cast<size_t>(ceil(sqrt(double(VECTOR_DIM))))),
    giantStepSize_(static_cast<size_t>(ceil(double(VECTOR_DIM) / double(babyStepSize_))))
{}

// -------------------- PLAINTEXT ROTATION --------------------

void DiagonalBSGSPreRotEnroller::applyPlaintextRotation(
    vector<double>& row, int amount, size_t vectorDim)
{
        ...
        ioThreads.emplace_back([&]() {
            while (true) {
                WorkItem item;
                {
                    ...
                    if (workQueue.empty() && encryptionDone.load()) break;
                    if (!workQueue.empty()) {
