...
DiagonalEnroller::DiagonalEnroller(CryptoContext<DCRTPoly> ccParam, PublicKey<DCRTPoly> pkParam, size_t vectorParam)
	...
	for (size_t i = 0; i < numOutputBatches; i++) {
		...
		for (size_t j = 0; j < matricesPerBatch; j++) {
			...
			if (matrixNum < matrices.size()) {
				copy(matrices[matrixNum][rowNum].begin(), matrices[matrixNum][rowNum].end(), currentBatch.begin() + rowIndex);
			}
		}
